...
def _sample(obss, acts, qs, max_pts, is_reweight, random_state: np.random.RandomState):
...
def identify_best_policy(env, policies, state_transformer, n_test_rollouts, verbose):
    if verbose:
...
def train_dagger(
    ...
    max_student = identify_best_policy(
        env, students, state_transformer, n_test_rollouts, verbose
    )
